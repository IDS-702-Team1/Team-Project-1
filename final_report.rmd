...
Unlike the relationship between age and 'treat' (Right), the relationship between education(left) and 'treat' is not linear. It indicates that some non-linear transformation should be performed on education. After evaluation, we decide to apply the quadratic transformation to the age variable.
...
Interaction between "married" and "age" was found in this step. Its p-value is slightly above 0.05 but strictly below 0.1 so the corresponding interaction is included. The ANOVA test also indicates that it would improve our model significantly. Hence it was preserved in our final model. 
