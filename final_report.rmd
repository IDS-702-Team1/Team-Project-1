...
Since the goal is to determine the effects of job training on salary increment, we take the response variable as the difference in salary between 1974 and 1978 and look at its distribution. Since, the distribution is quite normal, there is no transformation for the response variable. 
...
We then look at the trends between the response variables and other varaiables. We find that age shows a linear decreasing relation. While education seems to show a non-linear relation. The other categorical variables treat, black, hispan and married also seem to show differences with the differences in wages. However, with the variable non-degree we don't see much difference. Thereafter, we look at some potential interaction effects.
...
On the basis of the above, the logistic regression model is developed after considering the stepwise and forward7 built models using the AIC and BIC criteria and then selected. The Null Model used is ------- and the Full Model used is:------. Feeding these models, the AIC Forward Model found is: , the AIC Stepwise Model is: ,while with BIC Forward is: ---- and the BIC Stepwise Model is:------.

From the above we find that AIC forward model is the most statistically significant with p-value as: . We then compare this model with a number of other models including and excluding the variables.......... On doing the tests we find that the final model should be .

Intuitively too, this makes sense that changes in wages is impacted by age, education, ....
